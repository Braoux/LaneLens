...
function championFromSnapshot(snapshot: MatchupSelection, name: string): Champion | undefined { const normalized = name.trim().toLocaleLowerCase('fr-FR'); return SLOT_IDS.map((slot) => snapshot[slot]).find((champion) => champion.name.trim().toLocaleLowerCase('fr-FR') === normalized); }
...
function renderResult(snapshot: MatchupSelection, analysis: MatchupAnalysis, historical?: { readonly generatedAt: string }): void {
  ...
  if (historical) { const saved = element('p', 'saved-analysis-meta'); saved.textContent = `Analyse sauvegardée · ${formatHistoryDate(historical.generatedAt)}`; header.append(saved); }
  ...
  const early = element('section', 'early-timeline'); early.append(element('h3', undefined, 'Early game')); const earlyTrack = element('ol'); quick.early.forEach((line, index) => { const item = element('li'); item.append(element('span', undefined, `0${index + 1}`), element('p', undefined, line)); earlyTrack.append(item); }); early.append(earlyTrack); overlay.append(early);
  const target = element('article', 'target-block'); const targetChampion = championFromSnapshot(snapshot, quick.target.champion); if (targetChampion) target.append(portrait(targetChampion, 'target')); const targetCopy = element('div'); targetCopy.append(element('h3', undefined, 'Cible prioritaire'), element('strong', undefined, quick.target.champion), element('p', undefined, quick.target.explanation)); target.append(targetCopy); overlay.append(target);
  ...
  disclosure.append(detailBody); resultView.append(disclosure); disclosure.addEventListener('toggle', () => { summary.querySelector('span')!.textContent = disclosure.open ? 'Réduire l’analyse détaillée' : 'Voir l’analyse détaillée'; summary.querySelector('i')!.textContent = disclosure.open ? '↑' : '↓'; });
}
